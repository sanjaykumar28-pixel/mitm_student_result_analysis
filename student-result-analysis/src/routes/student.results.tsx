...
function StudentResults() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([studentService.getResults(), studentService.getDashboard()])
      .then(([resData, dashData]) => {
        if (cancelled) return;
        setSemesters(resData.semesters);
        setDashboard(dashData);
        setSem(
          resData.semesters.length
            ? resData.semesters[resData.semesters.length - 1].semester
            : null,
        );
        setError(null);
      })
  ...
  const subjects = useMemo(() => semData ? semData.subjects : [], [semData]);
  ...
  return (
    <div className="flex flex-col gap-6 pb-8 animate-in fade-in duration-500">
      ...
      {error ? (
        ...
        <div
          id="print-grade-card"
          className="bg-white text-black p-6 md:p-8 max-w-4xl mx-auto border shadow-sm rounded-xl"
        >
          {/* Header */}
            ...
            <table className="w-full text-xs border-collapse border border-black table-fixed">
              <thead>
                <tr className="bg-gray-100/80">
                  ...
                  <th className="border border-black p-2 text-left font-bold w-[24%]">
                    Course Name
                  </th>
                  ...
                  <th className="border border-black p-2 text-center font-bold w-[9%]">
                    Grade Point
                  </th>
                ...
                <tr className="font-bold">
                  ...
                  <td className="border border-black p-1.5 text-center" colSpan={2}>
                    {semData?.cgpa != null
                      ...
                      : dashboard?.overall_cgpa != null
                        ? dashboard.overall_cgpa.toFixed(2)
                        : "—"}
                  </td>
                </tr>
              </tbody>
          ...
          <div className="flex justify-end gap-8 text-xs font-bold my-5 py-2.5 border-y border-black/40">
            <div>
              Total Points : <span className="font-mono font-normal">{totalPoints}</span>
            </div>
            <div>
              SGPA :{" "}
          ...
          <p className="text-[10px] text-justify font-medium leading-tight">
            <span className="font-bold">Note:</span> These are provisional results. The final
            official results will be provided by the university/institution. Any discrepancies
            should be reported to the Controller of Examinations immediately.
          </p>
        </div>
      )}
    </div>
