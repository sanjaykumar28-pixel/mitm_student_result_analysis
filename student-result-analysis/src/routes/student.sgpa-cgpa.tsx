...
interface SemRow { id: number; semester: number; sgpa: number; credits: number }

const grades: Grade[] = ["O", "A+", "A", "B+", "B", "C", "F"];

function isGrade(value: string | null): value is Grade {
  return !!value && value in gradePoint;
}
      ...
      {error ? (
        <Card>
          <CardContent className="p-12 text-center">
            <p className="text-sm font-medium">SGPA / CGPA unavailable</p>
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Badge className="bg-primary/10 text-primary text-base">{sgpa == null ? "—" : sgpa.toFixed(2)}</Badge>
              </CardHeader>
              <CardContent className="space-y-3">
                {loaded && rows.length === 0 ? (
                  <p className="py-6 text-center text-sm text-muted-foreground">No subject data available to calculate SGPA.</p>
                ) : (
                  rows.map((r, i) => (
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Badge className="bg-primary/10 text-primary text-base">{cgpa == null ? "—" : cgpa.toFixed(2)}</Badge>
              </CardHeader>
              <CardContent className="space-y-3">
                ...
                {loaded && sems.length === 0 ? (
                  <p className="py-6 text-center text-sm text-muted-foreground">No semester data available to calculate CGPA.</p>
                ) : (
                  sems.map((s, i) => (
                ...
                <Button variant="outline" size="sm"
                  onClick={() => setSems((ss) => [...ss, { id: Date.now(), semester: ss.length + 1, sgpa: 0, credits: 0 }])}
                >
